    ...
    components: {
      schemas: {
        ...
        FrameworkSpec: {
          type: 'object',
          description:
            'The Well-Installed Framework, criterion by criterion. Cite as "Well-Installed Framework v<version>, criterion <id>" — an id is stable across editions and a version bump is a contract change, not a content edit. Every criterion names the paper section it derives from, and the build fails if that section does not exist.',
          required: ['meta', 'name', 'version', 'pillars', 'refuses'],
          properties: {
            meta: ref('ListMeta'),
            name: { type: 'string' },
            version: { type: 'string' },
            ...
            citation: { type: 'string', description: 'The citation form to use.' },
            note: { type: 'string' },
            refuses: { type: 'array', items: { type: 'string' } },
            severity_counts: { type: 'object' },
            how_to_use: { type: 'string' },
            pillars: { type: 'array', items: { type: 'object' } },
            self_assessment: { type: 'string', format: 'uri' },
            quote_scope_checklist: { type: 'string', format: 'uri' },
          },
        },
        CatalogueIndex: {
          type: 'object',

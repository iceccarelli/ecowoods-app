'use client';

import { useState, type FormEvent } from 'react';
import { BUSINESS_NAP } from '@ecowoods/shared/constants';

const MAX_FILES = 3;

export function SubmitForm({ orderId, maskedEmail }: { orderId: string; maskedEmail: string }) {
  const [state, setState] = useState<'idle' | 'sending' | 'sent' | 'error'>('idle');
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [files, setFiles] = useState<File[]>([]);

  function onPick(list: FileList | null) {
    if (!list) return;
    setFiles(Array.from(list).slice(0, MAX_FILES));
  }

  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const form = e.currentTarget;
    const fd = new FormData(form);
    fd.set('orderId', orderId);
    fd.delete('documents');
    for (const f of files) fd.append('documents', f, f.name);

    setState('sending');
    setErrors({});
    try {
      const res = await fetch('/api/well-installed-review/submit', {
        method: 'POST',
        headers: { 'x-requested-with': 'fetch', accept: 'application/json' },
        body: fd,
      });
      const json = await res.json().catch(() => ({}));
      if (res.ok) {
        setState('sent');
        form.reset();
        setFiles([]);
        return;
      }
      if (json.fieldErrors) {
        setErrors(json.fieldErrors);
        setState('idle');
        return;
      }
      setErrors({ form: json.message ?? 'Something went wrong.' });
      setState('error');
    } catch {
      setErrors({ form: 'Something went wrong.' });
      setState('error');
    }
  }

  if (state === 'sent') {
    return (
      <div className="ef ef--done wir-done" aria-live="polite">
        <h3 className="ef-h">Received.</h3>
        <p className="ef-intro">
          A senior estimator will read it and reply in writing by your tier&rsquo;s turnaround. Urgent?
          Call <a href={BUSINESS_NAP.phoneHref}>{BUSINESS_NAP.phoneDisplay}</a>.
        </p>
      </div>
    );
  }

  return (
    <form className="ef-form wir-submit-form" onSubmit={onSubmit} noValidate>
      <p className="tlx-note">Paid by {maskedEmail}. Confirm the same email below to attach your documents.</p>

      <label className="ef-field">
        <span>Your quote(s) — up to 3 files in total: one PDF per quote, or photos</span>
        <input
          name="documents"
          type="file"
          accept="application/pdf,image/jpeg,image/png,image/webp,image/heic,image/heif"
          multiple
          onChange={(e) => onPick(e.currentTarget.files)}
        />
        {files.length > 0 && (
          <em className="ef-photo-list">
            {files.map((f) => `${f.name} (${Math.max(1, Math.round(f.size / 1024))} KB)`).join(' · ')}
          </em>
        )}
        {errors.documents && <em className="ef-err">{errors.documents}</em>}
      </label>

      <label className="ef-field">
        <span>Confirm your email</span>
        <input name="email" type="email" autoComplete="email" required placeholder="jane@example.com" />
        {errors.email && <em className="ef-err">{errors.email}</em>}
      </label>

      <label className="ef-field">
        <span>
          Anything you already suspect <em>optional</em>
        </span>
        <textarea name="message" rows={2} placeholder="They quoted a screen and recoat but the finish is worn through in the hallway." />
      </label>

      <div className="ef-actions">
        <button type="submit" className="btn btn-copper" disabled={state === 'sending'}>
          {state === 'sending' ? 'Sending…' : 'Send the quote'}
        </button>
      </div>

      {(state === 'error' || errors.form) && (
        <p className="ef-err ef-err--block" role="alert">
          {errors.form ?? 'Something went wrong.'} Please call {BUSINESS_NAP.phoneDisplay} if it happens again.
        </p>
      )}
    </form>
  );
}

...
const LEAF = (
  <svg viewBox="0 0 32 32" width="18" height="18" aria-hidden="true">
    <path d="M16 5c-3.4 4.2-5.6 6.5-5.6 10.1 0 3.3 2.5 5.7 5.6 5.7s5.6-2.4 5.6-5.7C21.6 11.5 19.4 9.2 16 5Z" fill="#f6ecdd" fillOpacity="0.95" />
    <path d="M16 12.6c-1.7 2.3-2.5 3.5-2.5 5.3 0 1.8 1.1 3 2.5 3" fill="none" stroke={C.bronze} strokeWidth="1.4" strokeLinecap="round" />
  </svg>
);

const QUICK_REPLIES = [
  'Get a ballpark estimate',
  'Which species suits pets & kids?',
...
export default function ChatWidget() {
  ...
  // gentle nudge if they haven't opened it
  useEffect(() => {
    if (open) return;
  ...
  return (
    <>
      <style>{`
        ...
        .rg-send:hover { filter:brightness(1.06); }
      ...
      <div style={{ position: 'fixed', bottom: 22, right: 22, zIndex: 60, display: 'flex', alignItems: 'flex-end', gap: 10 }}>
        {!open && showNudge && (
          <div role="status" style={{ animation: 'rg-pop .3s ease', maxWidth: 220, background: C.paper, color: C.brown, border: `1px solid ${C.border}`, borderRadius: 14, padding: '10px 12px', fontSize: 13.5, lineHeight: 1.35, boxShadow: '0 10px 30px rgba(67,48,31,.14)', position: 'relative' }}>
        ...
        {!open && (
          <button
            ...
            aria-label="Chat with RenoGuide"
            style={{ height: 60, width: 60, borderRadius: 999, border: 'none', cursor: 'pointer', background: `linear-gradient(135deg, ${C.bronze}, ${C.bronzeDark})`, display: 'grid', placeItems: 'center', animation: 'rg-breathe 3.2s ease-in-out infinite', transition: 'transform .15s ease' }}
          >
      ...
      {open && (
        <div
          role="dialog"
          aria-label="RenoGuide chat"
          style={{ position: 'fixed', zIndex: 60, bottom: 'max(22px, env(safe-area-inset-bottom))', right: 22, width: 'min(94vw, 392px)', height: 'min(78vh, 600px)', display: 'flex', flexDirection: 'column', background: C.cream, borderRadius: 22, overflow: 'hidden', border: `1px solid ${C.border}`, boxShadow: '0 24px 60px rgba(67,48,31,.28)', animation: 'rg-pop .26s cubic-bezier(.2,.9,.3,1)' }}
        >
          {/* header */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '14px 16px', background: `linear-gradient(135deg, ${C.bronze}, ${C.bronzeDark})`, color: '#fff' }}>
            <span style={{ height: 38, width: 38, borderRadius: 11, background: 'rgba(255,255,255,.16)', display: 'grid', placeItems: 'center' }}>{LEAF}</span>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: 'var(--font-fraunces, Georgia), serif', fontSize: 17, fontWeight: 600, letterSpacing: .2 }}>RenoGuide</div>
          ...
          <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '16px 14px', background: C.cream }}>
            {messages.length === 0 && (
              <div style={{ animation: 'rg-pop .3s ease' }}>
                <div style={{ background: C.paper, border: `1px solid ${C.border}`, borderRadius: '4px 16px 16px 16px', padding: '12px 14px', fontSize: 14.5, color: C.brown, lineHeight: 1.5, boxShadow: '0 4px 14px rgba(67,48,31,.05)' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
                  {QUICK_REPLIES.map((q) => (
                    <button key={q} className="rg-chip" onClick={() => fire(q)} style={{ background: C.paper, border: `1px solid ${C.border}`, color: C.muted, borderRadius: 999, padding: '7px 13px', fontSize: 13, cursor: 'pointer', transition: 'all .15s ease' }}>{q}</button>
                  ))}
                </div>
              </div>
            )}
          ...
          <div style={{ borderTop: `1px solid ${C.border}`, background: C.paper, padding: 10 }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <input
                ...
                onKeyDown={(e) => e.key === 'Enter' && fire(input)}
                placeholder="e.g. white oak, ~600 sqft, M4K…"
                style={{ flex: 1, border: `1px solid ${C.border}`, background: C.cream, borderRadius: 12, padding: '11px 13px', fontSize: 14, color: C.brown, outline: 'none' }}
              />
              <button
                className="rg-send"
                onClick={() => fire(input)}
                ...
                aria-label="Send"
                style={{ height: 42, width: 42, borderRadius: 12, border: 'none', cursor: busy || !input.trim() ? 'default' : 'pointer', background: `linear-gradient(135deg, ${C.bronze}, ${C.bronzeDark})`, opacity: busy || !input.trim() ? 0.5 : 1, display: 'grid', placeItems: 'center', transition: 'filter .15s ease' }}
              >
            ...
            <div style={{ textAlign: 'center', fontSize: 10.5, color: C.muted, marginTop: 7, letterSpacing: .2 }}>
              Estimates are guidance — a specialist confirms on-site · Powered by Claude
            </div>
          </div>
        </div>
      )}

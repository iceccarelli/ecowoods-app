#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# apply-pricing-mobile.sh
#
# Makes the pricing section responsive: desktop keeps the 3-up grid; mobile
# switches to the shared SwipeDeck (the same one-thumb engine used by Services,
# Reviews, Process and Gallery), opening on the featured "Full Sand & Finish"
# tier. Three tall stacked cards on a phone were a marathon scroll; now it's one
# swipe, consistent with the rest of the site.
#
# Overwrites the existing PricingSection.tsx and appends deck-card CSS.
# Idempotent. Run from repo root.
# ---------------------------------------------------------------------------
set -euo pipefail
[ -f apps/web/app/components/PricingSection.tsx ] || { echo "ERROR: PricingSection.tsx not found — merge the pricing section first"; exit 1; }
grep -q 'useIsMobile' apps/web/app/components/SwipeDeck.tsx || { echo "ERROR: SwipeDeck not found"; exit 1; }
...
PXEOF
echo "  ~ PricingSection.tsx -> responsive (grid + SwipeDeck)"

python3 - << 'PY'
import sys
css = 'apps/web/app/globals.css'
c = open(css).read()
if '.pfd-card--price' in c:
    print("  = deck-card styles (present)")
else:
    c += r'''

/* ============================================================
   PRICING on mobile — the shared SwipeDeck carries the tiers.
   The card visual is the same .price-card; these rules adapt it to the deck's
   full-bleed dark card so it reads like the other swipe decks.
   ============================================================ */
.pfd-card--price {
  padding: 0;
...
.price-card--deck.is-featured {
  border-color: var(--copper);
  box-shadow: 0 0 0 1px var(--copper), var(--shadow-md);
}
/* the "Most common" flag needs room inside the deck card */
.price-card--deck .price-flag {
  top: 1rem;
  left: 1.6rem;
}
.price-card--deck .price-card-head {
  margin-top: 0.4rem;
}
'''
    open(css, 'w').write(c)
    print("  ~ deck-card styles appended")

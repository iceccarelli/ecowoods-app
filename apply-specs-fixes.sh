#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# apply-specs-fixes.sh
#
# Patches the ALREADY-MERGED specs/coverage section in place. The earlier
# installers guard on "does this section exist" and skip — but these three
# fixes target the existing version, so they edit rather than re-add:
#
#   1. .sx-title  -> --font-display, sized as a label not a hero (the giant
#      three-line heading in the screenshot)
#   2. footer estimate CTA -> hidden < 768px (sticky-cta-mobile already carries
#      "Get Free Quote"; the footer copy is a third one on the same screen)
#   3. CoverageMap -> v5 (collision-solved labels/dots/badges), only if the
#      live file is older
#
# Content-matched, idempotent. Run from repo root:  bash apply-specs-fixes.sh
# ---------------------------------------------------------------------------
set -euo pipefail
...
# --- 1. .sx-title: replace whatever sizing is there with the display-font one
old_title = re.search(r'\.sx-title \{[^}]*\}', c)
if not old_title:
    print("  ! .sx-title not found — is the section merged? SKIPPED")
elif 'font-display' in old_title.group(0):
    print("  = .sx-title (already fixed)")
else:
    new_title = """.sx-title {
  margin: 0;
...
.sx-title .serif-italic {
  ...
  color: var(--copper-text);
    ...
    c = c.replace(old_title.group(0), new_title, 1)
    changed.append("sx-title -> display font, label size")

# --- 2. footer CTA hidden on mobile
if 'footer-cta { display: none' in c:
    print("  = footer CTA hide (already present)")
else:
    c += """

/* --- footer estimate CTA: desktop only ---
   .sticky-cta-mobile already carries "Get Free Quote" on phones (pinned), so the
   footer's full-width estimate button is a third copy of one action. Hide < 768px. */
@media (max-width: 767px) {
  .footer-cta { display: none; }
}
"""
    changed.append("footer-cta hidden < 768px")
...
# --- 3. CoverageMap v5, only if older
if grep -q 'solved against labels' apps/web/app/components/CoverageMap.tsx 2>/dev/null; then
  echo "  = CoverageMap (already v5)"
else
...
COVEOF
  echo "  ~ CoverageMap.tsx -> v5 (collision-solved)"
fi

...
const HOME_HUBS = [
  '/resources', '/whats-new', '/standards', '/market', '/data', '/library', '/papers', '/framework', '/framework/assess', '/guides', '/glossary',
...
/* ── 3b. PATH links in the chrome and in consent/auth surfaces ────────────
 *
 * Check 3 above verifies that every FRAGMENT anchor in the chrome points at a
 * real element. It never checked that a path href points at a real route, and
 * that gap cost four live 404s:
 *
 *   SiteFooter.tsx        href="/privacy" and href="/terms" — in the footer of
 *                         every page on the site
 *   CookieConsentBanner   href="/privacy" — the "learn more" link on a banner
 *                         whose own comment cites PIPEDA as the reason it exists
 *   RegisterForm.tsx      "I agree to the Privacy Policy and Terms of Service",
 *                         both linked, neither route present
 *
 * The dead footer link on 246 pages is the cheap part. A consent banner whose
 * policy link does not resolve cannot produce informed consent, and a
 * registration form asking for agreement to two documents that do not exist is
 * asking for agreement to nothing.
 *
 * The files checked are the ones where a dead link is not merely a broken link
 * but a broken promise: site chrome, the consent banner, and the auth forms.
 */
/**
 * A complete route set, built separately from `routes` above.
 *
 * That one is deliberately narrowed — it excludes private trees and route
 * groups because its question is "is this marketing page reachable". This
 * question is different: does the URL in this href resolve AT ALL. `/login` and
 * `/register` live under an `(auth)` group, which does not appear in the URL,
 * so a set that filters those out would report a working link as broken.
 */
...
/* Paths that are handled outside the App Router or by a rewrite, and so will
   ...
   here. */
const NON_ROUTE_PATHS = new Map([
  ['/llms.txt', 'route handler at app/llms.txt/route.ts'],
  ['/llms-full.txt', 'route handler'],
  ['/ai.txt', 'route handler'],
  ['/robots.txt', 'generated by app/robots.ts'],
  ['/sitemap.xml', 'generated by app/sitemap.ts'],
  ['/feed.xml', 'route handler'],
  ['/about.md', 'markdown edition, served by a rewrite in next.config.js'],
]);

for (const [file, text] of sources) {
  if (!PROMISE_SURFACES.test(file)) continue;
  const clean = stripComments(text);
  const seen = new Set();
  for (const m of clean.matchAll(/href=(?:["']|\{["'`])(\/[a-z0-9\-/.]*)(?:["'`]\}|["'])/gi)) {

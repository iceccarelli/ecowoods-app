...
console.log('\n=== AXE VIOLATIONS (unique rule, by cell count) ===');
const a={};R.forEach(c=>(c.axe||[]).forEach(v=>{const k=`${v.impact||'?'}  ${v.id}  (${v.nodes} nodes)  ${v.help}`;a[k]=(a[k]||0)+1;}));
Object.keys(a).length?Object.entries(a).sort((x,y)=>y[1]-x[1]).forEach(([k,n])=>console.log(`  x${String(n).padStart(3)}  ${k}`)):console.log('  (none / not run)');
